...
def setup():
    ...
    mkdir_output_cmd = f'mkdir -m777 {utils.OUTPUT_DIR_DOCKER}/{utils.REPORT_DIR}'
    mkdir_logs_cmd = f'mkdir -m777 {utils.OUTPUT_DIR_DOCKER}/{utils.LOGS_DIR}'
    ...
    logging.basicConfig(filename=utils.LOG_FILE, filemode='w', format='[%(name)-8s]  [%(levelname)-8s]  %(message)s', level=logging.DEBUG)
...
def start_container(tool, container):
    ...
    f = open(f'{utils.OUTPUT_DIR_DOCKER}/{utils.LOGS_DIR}/{tool}.log',"w")
...
def create_tool(tool, input_dir_host, output_dir_host, config_dir_host , to_clean, tool_values):
    ...
    output_volume_docker = utils.OUTPUT_DIR_DOCKER if config["output_volume_docker"] == "" else config["output_volume_docker"] 
    ...
    if os.path.isfile(output):
        utils.log_message(utils.msglvl.DEBUG, '<{}> clearing existing output file', tool)
        with open(output, "w"):
            pass
    ...
    try:
        ...
        utils.log_message(utils.msglvl.ERROR, 'Could not format options string "{}", {}', config["options"][option], repr(e))
    utils.log_message(utils.msglvl.DEBUG, '<{}> has option command "{}"', tool, option_cmd)
...
def run_tool_thread(tool, input_dir_host, output_dir_host, config_dir_host, runs, to_clean):
    ...
    utils.log_message(utils.msglvl.INFO, '<{}> started running', tool)
    ...
    utils.log_message(utils.msglvl.INFO, '<{}> finished running', tool)
