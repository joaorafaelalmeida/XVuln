...
def produce_sarif_reports(types):
    ...
    for t in types.split(","):
        for subdir, dirs, files in os.walk(utils.INPUT_DIR_DOCKER):
            for file in files:
                ext = os.path.splitext(file)[-1].lower()
                if ".sarif" in file and t in ["MD","HTML","PDF"]:
                    produce_single_sarif(os.path.join(subdir, file),utils.OUTPUT_DIR_DOCKER, t)
